...
class ChildTaskViewScreen extends StatefulWidget {
  const ChildTaskViewScreen({Key? key}) : super(key: key);
...
class _ChildTaskViewScreenState extends State<ChildTaskViewScreen> {
  int _currentTabIndex = 0; // 0: New Tasks, 1: Pending, 2: Completed

  final HaseelaService _haseelaService = HaseelaService();

  // Replace with actual IDs from authentication
  String _currentParentId = "parent001";
  String _currentChildId = "child001"; // Sara's tasks are stored under child001
  ...
  void _loadChildProfile() async {
    try {
      ...
      final child =
          await _haseelaService.getChild(_currentParentId, _currentChildId);
  ...
  void _discoverAvailableChild() async {
    try {
      ...
      for (String childId in possibleChildIds) {
        ...
        try {
          final child =
              await _haseelaService.getChild(_currentParentId, childId);
          if (child != null) {
            print(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<Task>>(
        stream:
            _haseelaService.getTasksForChild(_currentParentId, _currentChildId),
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            ...
            // If it's a Firebase connection error, show offline state
            if (snapshot.error.toString().contains('unavailable') ||
                snapshot.error.toString().contains('connection') ||
                snapshot.error.toString().contains('timeout')) {
          ...
          if (snapshot.connectionState == ConnectionState.waiting) {
            print('StreamBuilder - Still waiting for data...');
            return _buildLoadingState();
          }

          // Handle empty data gracefully
          if (snapshot.data == null) {
            print('StreamBuilder - Data is null');
            return _buildLoadingState();
          }

          final allTasks = snapshot.data ?? [];

          // If no tasks and we have data (even if empty), show offline message
          if (allTasks.isEmpty && snapshot.hasData) {
            return _buildOfflineState();
          }
          print('All Tasks Count: ${allTasks.length}');
          print(
              'All Tasks: ${allTasks.map((t) => '${t.taskName} - ${t.status}').toList()}');
          ...
          print('New Tasks: ${newTasks.length}');
          print('Pending Tasks: ${pendingTasks.length}');
          print('Done Tasks: ${doneTasks.length}');

          // Debug: Print all task statuses
          print('=== TASK STATUS DEBUG ===');
          for (var task in allTasks) {
            print(
                'Task: ${task.taskName} - Status: "${task.status}" - TaskStatus: ${task.taskStatus}');
          }
            ...
            children: [
              StreamBuilder<Wallet?>(
                stream: _haseelaService.getWalletForChild(
                    _currentParentId, _currentChildId),
              ...
              Expanded(
                child:
                    _buildCurrentTabContent(newTasks, pendingTasks, doneTasks),
              ),
            ],
          );
        },
  ...
  Widget _buildOfflineState() {
    ...
    final newTasks =
        mockTasks.where((task) => task.status.toLowerCase() == 'new').toList();
    final pendingTasks = mockTasks
        .where((task) => task.status.toLowerCase() == 'pending')
        .toList();
    final doneTasks =
        mockTasks.where((task) => task.status.toLowerCase() == 'done').toList();
    final mockWallet = Wallet(
      id: 'mock_wallet',
      totalBalance: 150.0,
      savingsBalance: 100.0,
      spendingsBalance: 50.0,
      streakCount: 3,
    ...
    return Column(
      children: [
        _buildHeader(mockTasks, doneTasks, mockWallet, doneTasks),
        _buildToggleButtons(),
        Expanded(
            child: _buildCurrentTabContent(newTasks, pendingTasks, doneTasks)),
      ],
    );
  }
  ...
  List<Task> _createMockTasks() {
    return [
      ...
      Task(
        ...
        allowance: 30.0,
        priority: 'high',
        dueDate: DateTime.now().subtract(Duration(hours: 2)),
        createdAt: DateTime.now().subtract(Duration(days: 2)),
        assignedBy:
            FirebaseFirestore.instance.collection('Parents').doc('parent001'),
        completedDate: DateTime.now().subtract(Duration(hours: 1)),
        completedImagePath: null,
        image: null,
      ),
      Task(
  ...
  Widget _buildHeader(List<Task> allTasks, List<Task> completedTasks,
      Wallet? wallet, List<Task> completedTasksForEarning) {
    final totalTasks = allTasks.length;
    final totalEarned =
        completedTasksForEarning.fold(0.0, (sum, task) => sum + task.allowance);
  ...
  Widget _buildCurrentTabContent(
      List<Task> newTasks, List<Task> pendingTasks, List<Task> doneTasks) {
    ...
    if (confirmed == true) {
      try {
        ...
        await _haseelaService.updateTaskStatus(
            _currentParentId, _currentChildId, task.id, 'pending');
  ...
  // Helper method to get image from either completedImagePath or image field
